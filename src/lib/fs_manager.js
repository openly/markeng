...
var FSManager, config, exclude, fs, fse, include, path, _;
...
FSManager = {
  ...
  getDirContentsRecursive: function(dir, includes, excludes) {
    var contents, subDirs;
    contents = FSManager.getDirContents(dir, includes, excludes);
    subDirs = _.map(contents, function(content) {
  ...
  createRecursiveDirs: function(basePath, dirPath) {
    ...
    if (splitDirs.length >= 1) {
      return FSManager.createRecursiveDirs(nextDir, splitDirs.join("/"));
    }
  },
  ...
  writeFile: function(file, contents) {
    return fs.writeFileSync(path.normalize(file), contents);
  },
  append: function(file, contents) {
    return fs.appendFileSync(path.normalize(file), contents);
  },
  rmrf: function(dir) {
    return fse.rmrfSync(dir);
  },
  cp: function(src, dest) {
    return fs.createReadStream(path.normalize(config.dir + "/" + src)).pipe(fs.createWriteStream(path.normalize(dest)));
  },
  cpr: function(src, dest) {
    return fse.copyRecursive(path.normalize(config.dir + "/" + src), path.normalize(dest), function() {});
  }
};

include = function(contents, includes) {
  var filtered;
  filtered = [];
  if (_.isArray(includes)) {
    _.each(includes, function(includeRule) {
      return filtered = _.union(filtered, include(contents, includeRule));
    });
  } else {
    if (includes instanceof RegExp) {
      filtered = _.filter(contents, function(content) {
...
exclude = function(contents, excludes) {
  var toExclude;
